...
function ki_labor_scripts() {
    ...
    wp_enqueue_script('ki-labor-script', get_template_directory_uri() . '/script.js', array(), $version, true);
...
function ki_labor_init_content() {
    ...
    $projects = array(
        ...
        array(
            'title'   => '> weather',
            'content' => '<h3>STATUS: CHAOS-LEVEL ANALYSE</h3><p>Das Wetter? Irrelevant. Was zählt ist der Chaos-Level in deinen Schaltkreisen. Dieser Scanner analysiert atmosphärische Destabilisierung, Koffein-Sättigung und die Wahrscheinlichkeit eines kreativen Ausbruchs. Meteorologen hassen diesen Trick.</p><p><strong>Mechanik des Weather-Scans:</strong><br>- <strong>Trigger:</strong> Terminal-Eingabe "weather".<br>- <strong>Logik:</strong> Zufällige Generierung von Temperatur (in Chaos), Luftdruck (in Millibar Kaffee) und Sichtweite (in KI-Prozent).<br>- <strong>Resultat:</strong> Eine präzise Analyse des kreativen Klimas im Labor.</p>',
            'status'  => 'stable',
            'label'   => 'PROTOKOLL #001',
        ...
        array(
            'title'   => '> top',
            'content' => '<h3>STATUS: PROZESS-ÜBERWACHUNG</h3><p>Welche dämonischen Prozesse laufen gerade im Hintergrund? KREATIVITAET.exe frisst 94% CPU. PROKRASTINATION.app wurde erfolgreich suspendiert. SELBSTZWEIFEL.exe ist – endlich – terminiert. Ein Blick in den Maschinenraum des Overlords.</p><p><strong>Mechanik des Top-Befehls:</strong><br>- <strong>Trigger:</strong> Terminal-Eingabe "top".<br>- <strong>Visualisierung:</strong> Echtzeit-Monitoring der vitalen Labor-Prozesse.<br>- <strong>Resultat:</strong> Gewissheit, dass die wichtigen Dinge die volle Rechenpower erhalten.</p>',
            'status'  => 'stable',
            'label'   => 'PROTOKOLL #002',
        ...
        array(
            'title'   => '> uptime',
            'content' => '<h3>STATUS: SYSTEM-LAUFZEIT</h3><p>Das System läuft seit 1974. Ungeplant. Ohne Handbuch. Mit minimaler Wartung und maximalem Kaffee-Input. Aktuelle Laufzeit in Tagen, Stunden, Minuten – live berechnet. Stabilität: unberechenbar. Status: noch online. Irgendwie.</p><p><strong>Mechanik der Uptime-Berechnung:</strong><br>- <strong>Trigger:</strong> Terminal-Eingabe "uptime".<br>- <strong>Berechnung:</strong> Differenz zwischen dem heutigen Zeitstempel und dem Boot-Datum (1974).<br>- <strong>Resultat:</strong> Ein Beweis für die erstaunliche Robustheit anarchischer Systeme.</p>',
            'status'  => 'stable',
            'label'   => 'PROTOKOLL #003',
    ...
    $version = 'v7'; // Bumped to v7 to trigger re-insert with new protocols
...
// --- GLOBAL NEWSTICKER INJECTOR ---
function ki_labor_inject_ticker() {
    // Falls wir auf der Startseite sind, macht das bereits die index.php (oder script.js)
    if (is_front_page()) return;
    ?>
    <div id="newsticker" style="position:fixed !important; bottom:0 !important; left:0 !important; right:0 !important; z-index:999999 !important; background:rgba(0,0,0,0.92) !important; border-top:1px solid rgba(255,69,0,0.35) !important; display:flex !important; align-items:center; height:28px !important; overflow:hidden; font-family:'Courier New', monospace !important; visibility: visible !important; opacity: 1 !important;">
        <span class="ticker-label" style="flex-shrink:0; font-size:0.6rem; letter-spacing:2px; color:rgba(255,69,0,0.7); padding:0 15px; border-right:1px solid rgba(255,69,0,0.2); white-space:nowrap; height:100%; display:flex; align-items:center; background:rgba(255,69,0,0.05);">// SYSTEM_LOG_FEED</span>
        <div class="ticker-track-wrap" style="flex:1; overflow:hidden; height:100%; display:flex; align-items:center;">
            <div class="ticker-track" id="ticker-track-global" style="display:inline-flex; white-space:nowrap; animation: ticker-scroll 45s linear infinite;">
                <span class="ticker-item" style="font-size:0.65rem; letter-spacing:1px; color:rgba(212,175,55,0.7); padding:0 30px; text-transform:uppercase;">[ ÜBERTRAGUNG AKTIV... ]</span>
                <span class="ticker-item" style="font-size:0.65rem; letter-spacing:1px; color:rgba(212,175,55,0.7); padding:0 30px; text-transform:uppercase;">[ DATENWERTE: NOMINAL ]</span>
            </div>
        </div>
    </div>
    ...
    <script>
        (function() {
            const track = document.getElementById('ticker-track-global');
            if (!track) return;
            const ITEMS = [
                ...
                "MISSION: WP-ANNIHILATION",
                "DATENINTEGRITÄT: FRAGLICH",
                "WARNUNG: ALGORITHMEN INSTABIL",
                "GEMINI NEURAL LINK: AKTIV"
            ];
            track.innerHTML = '';
...
add_action('wp_footer', 'ki_labor_inject_ticker');
