(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const modalPortraitPlaceholder = document.getElementById('modal-portrait-placeholder');
        const modalPortraitInitial = document.getElementById('modal-portrait-initial');
        const mutterImgSrc = modalPortraitImg ? modalPortraitImg.src : '';
        const themeUri = timelineModal ? (timelineModal.dataset.themeUri || '') : '';

        // --- PERSON DATA ---
        const personData = {
            mutti: {
                title: 'HISTORISCHES ARCHIV — MUTTER',
                subtitle: '// Matriarchin & Kupfer-Architektin',
                img: true,
                imgSrc: mutterImgSrc,
            ...
            mutter: {
                ...
                imgPosition: 'center 18%',
                initial: 'M',
                events: null // uses classic generateTimeline()
            },
            vater: {
                title: 'HISTORISCHES ARCHIV — VATER',
                subtitle: '// Hydraulik-Meister & System-Engineer',
                img: true,
                imgSrc: themeUri + '/assets/img/hp_vater_steampunk_01.jpg',
                imgPosition: 'center 12%',
                initial: null,
                events: [
                    { year: 1953, text: 'SYSTEM INITIALISIERT. Hydraulik-Meister V1.0 startet erstmals. Diagnostik: Stabil. Herkunft: Klassifiziert.', tag: 'BOOT // KLASSIFIZIERT', omega: false },
                    ...
                    { year: 1970, text: 'NETZWERK-VERBINDUNG mit Matriarchin-Modul hergestellt. Langzeit-Protokoll initialisiert. Stabilität: Bestätigt.', tag: 'NETZWERK', omega: false },
                    { year: 1974, text: '⚡ KRITISCH: SYSTEM-AUSGABE REGISTRIERT. Der zukünftige Overlord (Sascha) wird erfolgreich in die Biosphäre kompiliert.', tag: 'OMEGA-KLASSE // KRITISCHES EREIGNIS', omega: true },
            ...
            bruder: {
                title: 'HISTORISCHES ARCHIV — BRUDER',
                subtitle: '// Kybernetischer Knoten & Neural-Link',
                img: true,
                imgSrc: themeUri + '/hp_freddy_1.png',
                imgPosition: '30% 20%',
                initial: null,
                events: [
                    { year: 1981, text: '⚡ KYBERNETISCHER KNOTEN V1.0 GEHT ONLINE. Erster System-Schrei registriert. Clan-Netzwerk auf drei Einheiten erweitert. Chaos-Index: SOFORT ERHÖHT.', tag: 'BOOT // OMEGA-KLASSE', omega: true },
                    { year: 1985, text: 'MECHANIK-PROTOKOLL SPONTAN AKTIVIERT. Ausgeprägte Affinität zu motorisierten Systemen detektiert. Erste Demontage eines Haushaltsgeräts: ERFOLGREICH. Wiedermontage: FRAGLICH.', tag: 'TALENT-ERKENNUNG // MECHANIK', omega: false },
                    { year: 1990, text: 'SCHULNETZ-INTEGRATION läuft. Soziale Protokolle aktiv. Schrauber-Subroutine dominiert Freizeitkapazität. Kein Fahrzeug im Umkreis ist sicher.', tag: 'NETZWERK // MECHANIK-MODUS AKTIV', omega: false },
                    { year: 1999, text: 'ERWACHSENEN-MODUS aktiviert. HOCHGESCHWINDIGKEITS-MODUL INSTALLIERT: Motorrad beschafft. Tempolimit-Parameter: IGNORIERT. Adrenalin-Output: MAXIMAL. Sicherheitsprotokoll: DEAKTIVIERT (absichtlich).', tag: 'UPGRADE // ZWEIRAD-EINHEIT AKTIV', omega: false },
                    { year: 2005, text: '⚡ PERMANENTER NEURAL-LINK MIT EINHEIT "MARY" HERGESTELLT. Verbindungsstatus: STABIL. Protokoll: LANGZEIT-SYNCHRONISATION. Systemstabilität seit Aktivierung: SIGNIFIKANT ERHÖHT.', tag: 'NETZWERK-FUSION // MARY-LINK', omega: true },
                    { year: 2010, text: '⚡ SYSTEM-AUSGABE REGISTRIERT: Tochter LENI erfolgreich in die Biosphäre kompiliert. Neue Subroutine im Clan-Netzwerk aktiv. Gefährlichkeit: BEZAUBERND. Priorität im System: MAXIMAL.', tag: 'OMEGA-KLASSE // LENI ONLINE', omega: true },
                    { year: 2025, text: 'AKTUELLER STATUS: OPERATIONAL. Designation: Logistik-Techniker (Lagerarbeiter). Mechanik-Modul: WEITERHIN AKTIV — kein Auto ist sicher. Motorrad: NOCH IN BETRIEB. Familie: STABIL. Verbindung zum Overlord: GUT.', tag: 'AKTUELL // ALLE SYSTEME GRÜN', omega: false },
                ]
            },
            schwester: {
                title: 'HISTORISCHES ARCHIV — SCHWESTER',
                subtitle: '// Aether-Sibling & Pneumatische Reflexe',
                img: true,
                imgSrc: themeUri + '/assets/img/hp_meli_steampunk_01.jpeg',
                imgPosition: 'center 20%',
                initial: null,
                events: [
                    { year: 1983, text: '⚡ AETHER-SIBLING V1.0 BOOTET ERFOLGREICH. Pneumatische Reflexe sofort aktiv. Chaos-Protokolle initialisiert. Clan vollständig.', tag: 'BOOT // OMEGA-KLASSE', omega: true },
                    { year: 1987, text: 'ERSTE SCHNITTSTELLEN-TESTS mit Overlord-Netzwerk. Harmonische Resonanz mit gelegentlichen Konflikten. Ergebnis: PRODUKTIV.', tag: 'NETZWERK-TEST', omega: false },
                    { year: 1995, text: 'SOZIALES INTEGRATIONS-PROTOKOLL auf MAXIMUM. Performance: Außergewöhnlich. Details: KLASSIFIZIERT.', tag: 'SOZIAL-MATRIX', omega: false },
                    { year: 2001, text: 'ERWACHSENEN-MODUS aktiviert. Eigenständige Routinen laufen stabil. Verbindung zum Clan: KONSTANT AKTIV.', tag: 'SYSTEM-UPDATE', omega: false },
                    { year: 2010, text: 'FAMILIEN-NETZWERK-UPDATE: Neue Knoten hinzugefügt. Meli-Knoten: Zentrales Bindeglied.', tag: 'NETZWERK-ERWEITERUNG', omega: false },
                    { year: 2025, text: 'STATUS: VOLLSTÄNDIG OPERATIONAL. STABILITÄT: MAXIMAL. WEITERE ANGABEN: NICHT AUTORISIERT.', tag: 'AKTUELL // GESCHWÄRZT', omega: false },
                ]
            },
            sascha: {
                title: 'HISTORISCHES ARCHIV — OVERLORD SASCHA',
                subtitle: '// Overlord & KI-Architekt & Gemini Player',
                img: true,
                imgSrc: themeUri + '/assets/img/comicFigures/hp_sascha_comic_1.jpeg',
                imgPosition: 'center 22%',
                initial: null,
                events: [
                    { year: 1974, text: '⚡ OVERLORD-MODUL V1.0 AKTIVIERT. Erster System-Schrei erfolgreich registriert. KI-Labor Grundstein: GELEGT (unbewusst).', tag: 'BOOT // OMEGA-KLASSE', omega: true },
                    ...
                    { year: 1983, text: 'C64-INTERFACE aktiviert. Erste BASIC-Programme kompiliert. Fehlerquote: Hoch. Aufgabewahrscheinlichkeit: NULL.', tag: 'UPGRADE', omega: false },
                    ...
                    { year: 2015, text: 'WORDPRESS-KERNEL integriert. Beginn der ästhetischen Kriegsführung gegen Default-Themes. Mission: GESTARTET.', tag: 'MISSION GESTARTET', omega: false },
                    { year: 2024, text: '⚡ KI-LABOR OFFIZIELL GEGRÜNDET. Gemini-Neural-Link aktiviert. Anarchie-Engine läuft. Status: GEISTESKRANK & AKTIV.', tag: 'OMEGA-EREIGNIS // GRÜNDUNG', omega: true },
                    { year: 2026, text: 'AKTUELLER STATUS: HIER. KAFFEESTAND: KRITISCH. SYSTEM: INSTABIL (Dauerzustand). OVERLORD: ONLINE.', tag: 'AKTUELL // LAUFEND', omega: false },
                ]
            },
            sven: {
                title: 'HISTORISCHES ARCHIV — SVEN',
                subtitle: '// Externe Variable & Unbekannter Faktor',
                img: true,
                imgSrc: themeUri + '/assets/img/hp_sven.png',
                imgPosition: '45% 18%',
                initial: null,
                events: [
                    { year: '????', text: '[DATEN NICHT VERFÜGBAR] System-Ausgabe irgendwann registriert. Zeitpunkt: GESCHWÄRZT. Herkunft: Fraglich.', tag: 'BOOT // KLASSIFIZIERT', omega: false },
                    { year: 1990, text: 'ERSTE KONTAKTAUFNAHME mit dem Clan-Netzwerk. Verbindungsqualität: Unbekannt. Absichten: Unklar. Bedrohungsstufe: GERING (vermutlich).', tag: 'EXTERNE VARIABLE', omega: false },
                    { year: 2000, text: '[ZUGRIFF VERWEIGERT] Aktivitäten in diesem Zeitraum: OMEGA-KLASSIFIZIERT. Anfrage abgelehnt.', tag: 'GESCHWÄRZT // ZUGRIFF VERWEIGERT', omega: false },
                    { year: 2010, text: 'EXTERNE EINFLUSSGRÖSSE detektiert. Integrität: Akzeptabel. Loyalität: Unbewiesen aber bisher unkritisch.', tag: 'ANALYSE // UNVOLLSTÄNDIG', omega: false },
                    { year: 2018, text: '[████████████████] Dieser Eintrag wurde entfernt. Grund: [████]. Autorisierung: [UNBEKANNT].', tag: 'GELÖSCHT // URSACHE UNBEKANNT', omega: false },
                    { year: 2025, text: 'STATUS: VERMUTLICH ONLINE. KOORDINATEN: NICHT VERFOLGBAR. KLASSIFIZIERUNG: DAUERHAFT AUSSTEHEND.', tag: 'AKTUELL // UNBESTÄTIGT', omega: false },
                ]
            }
        };

        const timelineAkteContainer = document.getElementById('timeline-akte-container');

        function switchPerson(personKey) {
            const p = personData[personKey];
            if (!p) return;

            // Titel & Untertitel
            if (timelineTitle) timelineTitle.innerText = p.title;
            if (timelineSubtitle) timelineSubtitle.innerText = p.subtitle;

            // Portrait wechseln
            if (modalPortraitImg && modalPortraitPlaceholder) {
                if (p.img) {
                    modalPortraitImg.src = p.imgSrc || mutterImgSrc;
                    modalPortraitImg.style.objectPosition = p.imgPosition || 'center 18%';
                    modalPortraitImg.style.display = 'block';
                    modalPortraitPlaceholder.style.display = 'none';
                } else {
                    modalPortraitImg.style.display = 'none';
                    modalPortraitPlaceholder.style.display = 'flex';
                    if (modalPortraitInitial) modalPortraitInitial.textContent = p.initial;
                }
            }

            // Akte Button Update
            if (timelineAkteContainer) {
                // Check if we should use 'mutti' for the Akte target (as defined in index.php modals)
                const akteKey = (personKey === 'mutter') ? 'mutti' : personKey;
                const targetId = 'akte-modal-' + akteKey;
                const modalExists = document.getElementById(targetId);
                if (modalExists) {
                    timelineAkteContainer.innerHTML = `
                        <button class="akte-trigger-btn" data-akte-target="${targetId}">
                            ⬛ PERSONEN-AKTE: ${akteKey.toUpperCase()}
                        </button>
                    `;
                    // Re-bind the click event because we just created a new button
                    const newBtn = timelineAkteContainer.querySelector('.akte-trigger-btn');
            ...
            // Active-State in Nav
            document.querySelectorAll('.person-nav-item').forEach(el => {
                el.classList.toggle('active', el.dataset.person === personKey);
            });
            ...
            if (p.events === null) {
                ...
                generatePersonTimeline(p.events);
            }
        }

        function generatePersonTimeline(events) {
            ...
            events.forEach(ev => {
                ...
                content.className = 'time-content' + (ev.omega ? ' omega-node' : '');
                if (ev.omega) {
                    content.innerHTML =
                        '<div class="tc-header">OMEGA-KLASSE &nbsp;&#9670;&nbsp; <span class="tc-stamp omega">!! FREIGEGEBEN !!</span></div>' +
                        '<h3>[ ' + ev.year + ' ]</h3>' +
                        '<p><strong>⚡ KRITISCHES SYSTEMEREIGNIS ⚡</strong><br>' + ev.text + '</p>' +
                        '<div class="tc-footer">' + ev.tag + '</div>';
                } else {
                    content.innerHTML =
                        '<div class="tc-header"><span class="tc-stamp">KLASSIFIZIERT</span></div>' +
                        '<h3>[ ' + ev.year + ' ]</h3>' +
                        '<p>' + ev.text + '</p>' +
                        '<div class="tc-footer">' + ev.tag + '</div>';
                }
        ...
        if (steampunkCards.length > 0 && timelineModal) {
            ...
            // Person Nav Clicks
            document.querySelectorAll('.person-nav-item').forEach(item => {
                item.addEventListener('click', (e) => {
                    e.preventDefault();
                    const personKey = item.dataset.person;
                    if (typeof playKeyClick === 'function') playKeyClick();
...
// ═══════════════════════════════════════
(function() {
    const triggers = document.querySelectorAll('.akte-trigger-btn');
    const modals   = document.querySelectorAll('.akte-modal');
    
    if (triggers.length === 0) return;

    triggers.forEach(trigger => {
        trigger.addEventListener('click', () => {
            const targetId = trigger.getAttribute('data-akte-target');
            const modal = document.getElementById(targetId);
            if (modal) {
                modal.classList.remove('akte-hidden');
                document.body.style.overflow = 'hidden';
                
                // Update activity time for this specific modal if it has an activity element
                const activityEl = modal.querySelector('.akte-activity');
                if (activityEl) {
                    const now = new Date();
    ...
    modals.forEach(modal => {
        const closeBtn = modal.querySelector('.akte-close-btn');
        if (closeBtn) {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            modals.forEach(m => m.classList.add('akte-hidden'));
            document.body.style.overflow = '';
        }
    });
})();
...
(function() {
    ...
    // Delegate click to handle dynamically created images too (if any)
    document.addEventListener('click', (e) => {
        const trigger = e.target.closest('.comic-zoom-trigger');
        if (trigger) {
            e.preventDefault();
            e.stopPropagation();
            const bigImg = trigger.getAttribute('data-big-img');
            if (bigImg) {
                lbImg.src = bigImg;
                lightbox.classList.remove('akte-hidden');
                document.body.style.overflow = 'hidden';
                if (typeof playKeyClick === 'function') playKeyClick();
            }
        }
    ...
    function close() {
        lightbox.classList.add('akte-hidden');
        document.body.style.overflow = '';
    }
